...
const VaultScreen = ({ navigation }) => {
	...
	// Firebase list all the file
	const getAllFiles = () => {
		const listRef = ref(storage, `vault/Administrator/`);
		listAll(listRef)
			.then((res) => {
			...
			<SafeAreaView className="h-full flex-col items-center ">
				<View className="w-full p-5">
					<View className="flex flex-row justify-start items-center">
						...
						<View className="ml-5">
							<Text className="text-xl font-bold">Your Files</Text>
							<Text className="text-xs">End-to-end encryption</Text>
						</View>
					</View>
				</View>

				<ScrollView>
					<View className="w-full flex flex-row flex-wrap jusitfy-evenly align-center gap-3 mt-5 p-5">
						{files ? (
							files.map((item, index) => (
								<View className="  w-full ">
									<FileIcon key={index} fileName={item.name} />
								</View>
							))
						) : (
